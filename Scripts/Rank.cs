...
public class Rank : MonoBehaviour {
    ...
    public static int Max = 1; //3; rodadas para média
	public static float timer = 0.7f;
	public static bool StartEnd = false;

    public static int Needed_to_pass = 10;


    public void UP(){
        if (Mathf.Round(Score.PinCount) >= (Needed_to_pass + Level)) { 
                Level++;
        }
        FindObjectOfType<Count>().Set(Current.ToString(), (Needed_to_pass + Level).ToString());
